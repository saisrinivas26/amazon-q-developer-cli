    ...
    pub fn start_capture(&self, audio_sender: mpsc::Sender<Vec<u8>>) -> Result<Stream> {
        ...
        let stream = match supported_config.sample_format() {
            cpal::SampleFormat::F32 => {
                self.build_input_stream_f32(&config, sender)?
            }
            cpal::SampleFormat::I16 => {
                self.build_input_stream_i16(&config, sender)?
            }
            cpal::SampleFormat::U16 => {
                self.build_input_stream_u16(&config, sender)?
            }
            sample_format => {
    ...
    fn build_input_stream_i16(&self, config: &StreamConfig, sender: mpsc::Sender<Vec<u8>>) -> Result<Stream> {
        ...
        let stream = self.device.build_input_stream(
            ...
            move |data: &[i16], _: &cpal::InputCallbackInfo| {
                ...
                // Convert to bytes (little-endian)
                let bytes: Vec<u8> = resampled_data
                    .iter()
                    .flat_map(|&sample| sample.to_le_bytes())
                    .collect();
